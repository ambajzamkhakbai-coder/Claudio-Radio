...
function assemblePrompt(options = {}) {
  ...
  const chatMemory = db.getChatMemoryContext({
    maxRecent: 8,
    maxMonthly: 20,
    maxSummaries: 6
  });
...
【长期压缩聊天记忆（超过 ${chatMemory.retentionDays} 天的内容只保留摘要）】
${chatMemory.summaryText}

【近 ${chatMemory.retentionDays} 天聊天记忆（用于保持上下文，不要脱离前文）】
${chatMemory.monthlyText}

【最近几轮实时对话原文（优先级最高）】
${chatMemory.recentText}
`;

  // 如果听众有最新主动输入，则追加输入上下文
  if (userInput) {
    promptBody += `\n【听众刚刚对你说的话】\n听众："${userInput}"\n（请根据听众的话语自主判断意图：如果听众只是闲聊/打招呼/吐槽/倾诉，intent 设为 "chat"，play 为空数组，结合聊天记忆像朋友一样自然回应，不要脱离上下文；如果听众明确表达了想听歌的意愿，intent 设为 "recommend"，在 play 中从曲库推荐 3 首候选歌曲并附带 reason。）`;
  }
  
  promptBody += `\n\n【核心约束硬规则】\n1. 必须在 JSON 中包含 "intent" 字段，值为 "chat"、"recommend" 或 "play" 之一。\n2. 当 intent 为 "recommend" 或 "play" 时，请务必且只能从上面的【电台当前海量真实备选曲库贴纸】中挑选歌曲！\n3. 当 intent 为 "chat" 时，play 必须为空数组 []，不要推荐歌曲。\n4. 坚决禁止凭空捏造任何不在曲库贴纸中的歌曲 ID！\n5. 请在返回的 JSON 对象中，将 play 数组内的歌曲 id、name、artist 完美对应曲库里的值输出。\n6. 回答闲聊时必须优先参考【最近几轮实时对话原文】和【近 30 天聊天记忆】，超过 30 天的压缩记忆只能作为背景参考，不要把压缩摘要当作逐字事实。`;
